...
public class RekamMedikDAO {
    ...
    public rekam_medik_model getRekamMedikById(String id_rekammedik)
    {
        System.out.println("-BY ID-");
        rekam_medik_model rmm=new rekam_medik_model();
        String query="CALL BYID_REKAMMEDIK(?)";
        try
        {
            ps=conn.prepareStatement(query);
            ps.setString(1, id_rekammedik);
            rs=ps.executeQuery();
            if(rs.next())
            {
                if(!rs.getString("id_pasien").equals(""))
    ...
    public static void main(String[] args) {
        ...
        rekam_medik_model rmm = new rekam_medik_model();
        rmm.setId_pasien("PS0001");
        rmm.setTgl_daftar("2020-10-5");
        rmm.setId_poli("P1");
        rmm.setTes_darah("Rendah");
        rmm.setBerat(55);
        rmm.setTinggi(170);
        ...
        rmm.setId_resep("RS0001");
        rmm.setDiagnosa("Vertigo");
        rmm.setUser("98234");
        //rmd.add(rmm, "update");
        System.out.println(rmd.getRekamMedikById("PS0001"));
